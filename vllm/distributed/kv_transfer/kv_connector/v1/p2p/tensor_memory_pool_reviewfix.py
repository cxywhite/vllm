...
class TensorMemoryPool:
    ...
    def __init__(self, max_block_size: int, min_block_size: int = 512):
        ...
        # [source] original implementation only had free_lists/allocated_blocks.
        # self.free_lists: dict[int, dict[int, MemoryBlock]] = {}
        # self.allocated_blocks: dict[int, MemoryBlock] = {}
        # [wt] avoid oom keep pool accounting lock-free for readers and
        # synchronized for updates.
    ...
    def allocate(self, size: int) -> int:
        ...
        # [source] original implementation was lock-free and without _used_bytes.
        ...
        with self._lock:
            ...
            while current_size <= self.max_block_size:
                if self.free_lists[current_size]:
                    ...
                    self._split_block(block, required_size)
                    self.allocated_blocks[block.addr] = block
                    # [wt] avoid oom track pool occupancy in O(1).
    ...
    def free(self, addr: int):
        ...
        with self._lock:
            ...
            block = self.allocated_blocks.pop(addr)
            # [wt] avoid oom release occupancy first to improve freshness.
            self._used_bytes = max(0, self._used_bytes - block.size)
            self._merge_buddies(block)
        # [end]
    ...
    def load_tensor(self, addr: int, dtype: torch.dtype, shape: tuple[int,
        ...
        """
        # [source] original implementation read allocated_blocks without lock.
        with self._lock:
            if addr not in self.allocated_blocks:
    ...
    def get_total_bytes(self) -> int:
        # [wt] avoid oom expose fixed pool capacity for proxy admission.
        return self.max_block_size

    def get_used_bytes(self) -> int:
        # [wt] avoid oom expose real-time pool occupancy.
        with self._lock:
            return self._used_bytes

    def get_free_bytes(self) -> int:
        # [wt] avoid oom expose real-time free bytes with O(1) cost.
        with self._lock:
            return max(0, self.max_block_size - self._used_bytes)

    def cleanup(self):
        """Cleans up all memory resources and resets the pool state."""
        # [source] original cleanup:
