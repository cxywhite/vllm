...
class P2pNcclEngine:
    ...
    def __init__(self,
        ...
        # [source] original state only tracked buffer bytes and threshold.
        # self.buffer_size = 0
        # self.buffer_size_threshold = float(self.config.kv_buffer_size)
        # [wt] avoid oom synchronize kv_buffer occupancy across recv thread
        # and request-processing thread with minimal lock scope.
        self.buffer_size_lock = threading.Lock()
        self.buffer_size = 0
        self.buffer_size_threshold = float(self.config.kv_buffer_size)
        # [wt] avoid oom send capacity updates to proxy with low admission
        # latency; default keeps overhead low.
        self.proxy_ping_interval_s = float(
            self.config.get_from_extra_config("proxy_ping_interval_s", "0.2"))
        # [end]
        ...
        logger.info(
            ...
            "threshold:%.2f, nccl_num_channels:%s, ping_interval_s:%.3f",
            ...
            self.proxy_ping_interval_s)

    def _build_capacity_report(self) -> dict[str, float]:
        # [wt] avoid oom export real-time decode temporary buffer capacity.
        with self.buffer_size_lock:
            buffer_used = float(self.buffer_size)
            buffer_cap = float(self.buffer_size_threshold)
        pool_total = float(self.pool.get_total_bytes())
        pool_used = float(self.pool.get_used_bytes())
        return {
            "kv_buffer_used_bytes": buffer_used,
            "kv_buffer_capacity_bytes": buffer_cap,
            "kv_buffer_free_bytes": max(0.0, buffer_cap - buffer_used),
            "pool_total_bytes": pool_total,
            "pool_used_bytes": pool_used,
            "pool_free_bytes": max(0.0, pool_total - pool_used),
            "capacity_ts": time.time(),
        }
        # [end]
    ...
    def recv_tensor(
        ...
        if self.send_type == "PUT" or self.send_type == "PUT_ASYNC":
            ...
            with self.recv_store_cv:
                ...
                # wt
                # Consume-on-read: remove the tensor immediately after it is
                # retrieved so connector-side buffers do not hold extra copies
            ...
            if tensor is not None:
                if isinstance(tensor, tuple):
                    ...
                    tensor = self.pool.load_tensor(addr, dtype, shape,
                                                   self.device)
                    # wt
                    # Release host-pool backing storage immediately after
                    # loading it to device memory.
                    self.pool.free(addr)
                else:
                    # [source] original logic directly decremented without lock.
                    # self.buffer_size -= (tensor.element_size() * tensor.numel())
                    # [wt] avoid oom keep kv_buffer occupancy accurate after
                    # consume-on-read.
                    tensor_bytes = tensor.element_size() * tensor.numel()
                    with self.buffer_size_lock:
                        self.buffer_size = max(0, self.buffer_size -
                                               tensor_bytes)
                    # [end]
            else:
        ...
        while True:
            ...
            if data["cmd"] == "NEW":
                ...
                try:
                    ...
                    self.recv(comm, tensor, rank ^ 1, self.recv_stream)
                    tensor_size = tensor.element_size() * tensor.numel()
                    # [source] original logic without lock:
                    # if (self.buffer_size + tensor_size > self.buffer_size_threshold):
                    #     addr = self.pool.store_tensor(tensor)
                    #     tensor = (addr, tensor.dtype, tensor.shape)
                    # else:
                    #     self.buffer_size += tensor_size
                    # [wt] avoid oom decide spill-to-pool based on synchronized
                    # buffer occupancy.
                    spill_to_pool = False
                    with self.buffer_size_lock:
                        if (self.buffer_size + tensor_size
                            ...
                            self.buffer_size += tensor_size
                    if spill_to_pool:
                        # [source] previous logic assumed pool allocation always succeeds:
                        # addr = self.pool.store_tensor(tensor)
                        # tensor = (addr, tensor.dtype, tensor.shape)
                        # [wt] avoid oom if pool allocation fails, keep worker alive
                        # and mark this tensor unavailable instead of crashing
                        # listen_for_requests thread.
                        try:
                            addr = self.pool.store_tensor(tensor)
                            tensor = (addr, tensor.dtype, tensor.shape)
                            logger.warning(
                            ...
                            tensor = None
                            logger.warning(
                                "🔴[PUT]Recv Tensor, Pool Allocation Failed, "
                                "%s👈%s, data:%s, err:%s", self.zmq_address,
                                remote_address.decode(), data, str(err))
                        except Exception:
                            tensor = None
                            logger.exception(
                                "🔴[PUT]Recv Tensor, Unexpected Spill Failure, "
    ...
    def ping(self):
        ...
        # [source] original ping payload and interval:
        # data = {
        ...
        while True:
            ...
            data.update(self._build_capacity_report())
            sock.send(msgpack.dumps(data))
            time.sleep(self.proxy_ping_interval_s)
        # [end]
