...
from customfunction import load_and_prepare_dataset,ensure_id_column_from_df,extract_first_human_prompt,save_outputs_grouped_csv_by_base_reqid
import pdb
...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom":
        ...
        # )
        if args.dataset_name == "custom":
            # 决定使用 CSV 还是 JSONL 分支
            use_csv = getattr(args, 'use_custom_csv', False)

            # 通用：加载模型配置以确定最大位置（max_pos），保留原有行为
            max_pos = 2048
            try:
                if AutoConfig is not None:
                    ...
                    print(f"加载模型配置成功，模型最大位置: {max_pos}")
            except Exception as e:
                print(f"加载模型配置失败，使用默认 max_pos={max_pos}: {e}")
                max_pos = 2048
            max_pos = min(max_pos, 131072)
            print(f"最终使用最大位置: {max_pos}")

            # tokenize_len helper（优先使用已取得的 tokenizer，失败回退到 AutoTokenizer，再失败回退到字符长度）
            def tokenize_len(text: str) -> int:
                ...
                try:
                    ...
                    return len(local_tok(text, add_special_tokens=False)['input_ids'])
                except Exception:
                    return len(text)

            # 全局 maxtokenscustom（fallback）
            maxtokenscustom_global = args.maxtokenscustom or 1024
            # max_embedding_pos 用于 CSV 分支（可由 args.max_embedding_pos 覆盖）
            max_embedding_pos = getattr(args, 'max_embedding_pos', max_pos)
            max_embedding_pos = min(max_embedding_pos, 131072)

            input_requests = []

            if use_csv:
                # ---------------- CSV 分支 ----------------
                # 按要求：从固定 CSV 加载，CSV 分支不做过长过滤也不做 num_prompts 抽样
                import pandas as pd
                csv_path = "/root/predict/predict_experiment/dataset_result/benchmarkserving_result/llama/single/results_20251110_073554_llama.csv"
                print(f"使用自定义 CSV 构造请求: {csv_path}")
                try:
                    csv_df = pd.read_csv(csv_path, dtype=str, keep_default_na=False)
                    print(f"已加载 CSV，共 {len(csv_df)} 条记录")
                except Exception as e:
                    raise RuntimeError(f"加载自定义 CSV 失败: {e}") from e

                # 容错：寻找 prompt 列
                if 'prompt' not in csv_df.columns:
                    alt_prompt_cols = [c for c in ['text', 'message', 'messages', 'dialogue'] if c in csv_df.columns]
                    if alt_prompt_cols:
                        csv_df = csv_df.rename(columns={alt_prompt_cols[0]: 'prompt'})
                        print(f"CSV 未包含 'prompt' 列，使用候选列 '{alt_prompt_cols[0]}'")
                    else:
                        raise ValueError("CSV 文件必须包含 'prompt' 列（或 'text','message','messages','dialogue' 中的一个）")

                # 容错：寻找 req_id 列，否则基于行号生成
                if 'req_id' not in csv_df.columns:
                    alt_id_cols = [c for c in ['id', 'request_id'] if c in csv_df.columns]
                    if alt_id_cols:
                        csv_df = csv_df.rename(columns={alt_id_cols[0]: 'req_id'})
                        print(f"CSV 未包含 'req_id' 列，使用候选列 '{alt_id_cols[0]}'")
                    else:
                        csv_df['req_id'] = [f"rowcsv{i}" for i in range(len(csv_df))]
                        print("CSV 未包含 'req_id' 列，已基于行索引生成 req_id")

                # 注意：按你要求，CSV 分支 **不** 过滤空 prompt（保留 CSV 原样）
                # 计算 prompt_tokens（CSV 分支不做过长过滤）
                csv_df['prompt_tokens'] = csv_df['prompt'].apply(lambda t: tokenize_len(str(t)))
                print("已计算 CSV 中每条记录的 prompt_tokens（CSV 分支不做过长过滤或 num_prompts 抽样）")

                # 计算 custom_max_tokens：max(1, min(max_embedding_pos - input_len, maxtokenscustom_global))
                def compute_custom_max_from_embedding(input_len: int) -> int:
                    available = max_embedding_pos - int(input_len)
                    return max(1, min(int(available), int(maxtokenscustom_global)))

                csv_df['custom_max_tokens'] = csv_df['prompt_tokens'].apply(lambda l: compute_custom_max_from_embedding(int(l)))

                # 输出与 JSONL 分支相同的样本长度统计（前3、后3、min/max/mean）
                try:
                    print(f'前3个CSV样本：{csv_df.head(3)}')
                    print(f'后3个CSV样本：{csv_df.tail(3)}')
                    print("\nCSV样本长度统计:")
                    print(f"  - 最小prompt长度: {int(csv_df['prompt_tokens'].min())}")
                    print(f"  - 最大prompt长度: {int(csv_df['prompt_tokens'].max())}")
                    print(f"  - 平均prompt长度: {float(csv_df['prompt_tokens'].mean()):.1f}")
                    print(f"  - 最小定制max_tokens: {int(csv_df['custom_max_tokens'].min())}")
                    print(f"  - 最大定制max_tokens: {int(csv_df['custom_max_tokens'].max())}")
                    print(f"  - 平均定制max_tokens: {float(csv_df['custom_max_tokens'].mean()):.1f}")
                except Exception as e:
                    print(f"输出 CSV 样本统计时发生错误（非致命）：{e}")

                # 读取重复次数 m（默认 1）
                m = getattr(args, "m", 1) or 1
                if m < 1:
                    m = 1

                # 构建 input_requests（CSV 分支），并为每条请求创建 sampling_params（与 JSONL 分支一致）
                for idx, row in csv_df.iterrows():
                    base_id = row.get('req_id') or f"rowcsv{idx}"
                    if base_id is None:
                        base_id = f"rowcsv{idx}"
                    prompt_text = str(row['prompt'])
                    prompt_len = int(row['prompt_tokens'])
                    expected_output_len = int(row['custom_max_tokens'])

                    # 采样参数（CSV 分支也生成，与 JSONL 分支一致）
                    # sampling_params = {
                    ...
                    for rep in range(m):
                        ...
                        input_requests.append(
                            SampleRequest(
                                ...
                                # 如果你的 SampleRequest 支持采样参数，请取消下一行的注释并传入 sampling_params
                                # sampling_params=sampling_params
                            )
                        )

                print(f"从 CSV 构建 input_requests 完成：CSV 原样本 {len(csv_df)} 条，复制次数 m={m}，总请求数 = {len(input_requests)}")

            else:
                # ---------------- JSONL/JSON 原始分支（保持你原有逻辑） ----------------
                print("【阶段1】加载并准备自定义数据集（.jsonl/.json）...")
                try:
                    df = load_and_prepare_dataset(args.dataset_path, split=None)
                    print(f"✓ 数据集加载成功，共 {len(df)} 条记录")
                except Exception as e:
                    raise RuntimeError(f"加载自定义数据集失败: {e}") from e

                # 如果没有 conversations 列，尝试寻找候选列
                if 'conversations' not in df.columns:
                    ...
                    for candidate in conversation_candidates:
                        if candidate in df.columns:
                            print(f"未找到 'conversations' 字段，但找到 '{candidate}'，将使用它。")
                    ...
                    if not found:
                        ...
                        raise ValueError(
                            "数据集缺少对话字段。需要包含以下任一字段: conversations, messages, dialogue, turns. "
                            f"可用字段: {available}"
                        )

                print("提取第一个 human prompt ...")
                ...
                print("【阶段2】计算 prompt token 长度并过滤过长 prompt ...")
                df['prompt_tokens'] = df['prompt'].apply(lambda x: tokenize_len(x))
                filtered_df = df[df['prompt_tokens'] < 8192].reset_index(drop=True)
                filtered_count = len(filtered_df)
                print(f"过滤长 prompt: {valid_count} -> {filtered_count} 条记录 (max_pos=8192)")
                if filtered_count == 0:
                    raise ValueError("过滤后没有可用的 prompt：所有 prompt 超过模型的最大长度限制。")

                # 确定抽样数量
                if filtered_count < args.num_prompts:
                    print(f"警告: 过滤后只有 {filtered_count} 条记录，少于请求的 {args.num_prompts} 条；将使用所有 {filtered_count} 条")
                ...
                # deepseek 计算 custom_max_tokens（保留原来的 deepseek 公式）
                if 1:
                    print(f'[warn]qwen！！！！！！！！！')
                ...
                print(f'前3个样本：{sample_df.head(3)}')
                print(f'后3个样本：{sample_df.tail(3)}')
                print("\n样本长度统计:")
                ...
                # 构建 input_requests（benchmark 侧代码通常接收一组请求对象/字典）
                for idx, row in sample_df.iterrows():
                    # sampling_params = {
                    #     "max_tokens": int(row['custom_max_tokens']),
                ...
                print(f"构建 input_requests 完成：原样本 {len(sample_df)} 条，复制次数 m={m}，总请求数 = {len(input_requests)}")
