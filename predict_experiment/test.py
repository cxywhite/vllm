...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom":
        ...
        # )
        if args.dataset_name == "custom":
            # 期望 args.dataset_path 指向本地 .jsonl 或 .json 文件路径
            ...
            # 用于计算 token 长度的 helper：优先使用已获得的 tokenizer（get_tokenizer 返回的对象），
            # 如果该 tokenizer 不支持直接编码/调用，则回退到 transformers.AutoTokenizer
            def tokenize_len(text: str) -> int:
                try:
                    # 如果 get_tokenizer 返回的对象可以被直接调用并返回 input_ids
                    maybe = tokenizer(text, add_special_tokens=True)
                    if isinstance(maybe, dict) and 'input_ids' in maybe:
                        return len(maybe['input_ids'])
                    # 有些 tokenizer 提供 encode 方法
            ...
            # 计算每个样本的 custom_max_tokens（受 args.maxtokenscustom 或默认 1024 控制）
            maxtokenscustom_global = args.maxtokenscustom or 1024
            # deepseek
            if 1:
                print(f'[warn]qwen！！！！！！！！！')
                sample_df['custom_max_tokens'] = sample_df['prompt_tokens'].apply(
                lambda input_len: max(1, min(32*1024 - input_len, maxtokenscustom_global))
                )
            # sample_df['custom_max_tokens'] = sample_df['prompt_tokens'].apply(
            #     lambda input_len: max(1, min(max_pos - input_len, maxtokenscustom_global))
            # )
            ...
            # 保存到指定.jsonl文件供后续使用
            # if args.custom_output_sample_path:
            #     try:
            #         save_df = sample_df.drop(columns=['prompt_tokens'])
            #         # 保存文件名加上时间戳以防覆盖
            #         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            #         out_file = f"{os.path.splitext(args.custom_output_sample_path)[0]}_{timestamp}.jsonl"
            #         save_df.to_json(out_file, orient='records', lines=True, force_ascii=False)
            #         print(f"抽样样本已保存到: {out_file}")
            #     except Exception as e:
            #         print(f"保存抽样样本失败: {e}")
            ...
            input_requests = []
            # 使用 iterrows 时 index 用作 fallback id
            for idx, row in sample_df.iterrows():
                ...
                # base id（若 row 中有 'id' 列则使用，否则用行索引）
                base_id = row.get('id', None)
                if base_id is None:
                    base_id = f"row{idx}"

                # 复制 m 次，每次生成一个唯一 req_id（在 base_id 后加上后缀）
                for rep in range(m):
