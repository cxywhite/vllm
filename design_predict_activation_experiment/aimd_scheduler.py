...
class AIMDScheduler(Scheduler):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        running_now=self.last_running_tokens+len(self.running) < self.physical_mem
        if running_now:
            pass
        else:
            # running中的请求按照num_computed_tokens/predictor_output_len从高到底排序，优先抢占那些比例较低的请求
            self.running.sort(key=lambda r: (r.num_computed_tokens / r.predictor_meta['predict_output_len']), reverse=True)
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            tmp_request_length_record.append((request.num_computed_tokens,request.predictor_meta['predict_output_len']))
            req_index += 1
        ...
        if self.waiting_times==0:
            if not preempted_reqs:
                # [WT]virture mem 2026-01-05 22:10:31
                ...
                while self.waiting and token_budget > 0:
                    ...
                    bisect.insort(tmp_request_length_record, (request.num_computed_tokens,request.predictor_meta['predict_output_len']), key=lambda x: -x[1])
                    ...
                    logger.info(f"[WT]*** len(tmp_request_length_record): {len(tmp_request_length_record)} bisect time cost: {time_end - time_start} s")
                    if need_max_token_num > self.virtual_mem:
                        logger.info(f'[WT]*** peek memory > virtual_mem: {need_max_token_num} > {self.virtual_mem}')
                        break
                    # logger.info(f'[WT]*** ok')
                ...
                if self.virtual_mem<self.physical_mem:
                    ...
                    self.virtual_mem+=4*self.max_model_len
            # [WT]virture mem 2026-01-05 22:45:47
            else:
                if self.virtual_mem>self.physical_mem:
                    logger.info(f'[WT]*** skip scheduling waiting requests due to virtual_mem > physical_mem: {self.virtual_mem} > {self.physical_mem}')
                    self.virtual_mem=self.physical_mem
        ...
        # [WT]virture mem 2026-01-06 00:13:11
            ...
            if self.waiting_times >0:
                ...
                logger.info(f'[WT]*** skip scheduling waiting requests due to waiting_times: {self.waiting_times}')
    ...
    def update_from_output(
        ...
        # [WT]virture mem 2026-01-06 00:17:36
        self.last_running_tokens=0
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # [WT]virture mem 2026-01-06 00:17:22
            else:
                self.last_running_tokens += request.num_computed_tokens
