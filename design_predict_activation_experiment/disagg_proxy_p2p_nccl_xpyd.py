...
# ============================================================
# [WT][ADD] Predictor metadata 接收（ZMQ）
# ============================================================

PREDICTOR_ZMQ_ADDR = "tcp://0.0.0.0:32323"
...
# ============================================================
# [WT][ADD] Predictor metadata 监听线程
# ============================================================
...
def _listen_predictor_metadata():
    while True:
        try:
            ...
            meta_list = json.loads(msg)  # 这是一个 metadata 字典的列表
            
            # 遍历列表中的每个 metadata 条目
            for meta in meta_list:
                # 修正字段名：使用 'req_id' 而不是 'request_id'
                req_id = meta.get("req_id")
                
                print(f"[WT][Proxy] Received predictor meta for {req_id}: {meta}")
                    
        except json.JSONDecodeError as e:
            print(f"[WT][Proxy] JSON decode error: {e}, raw message: {msg}")
        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"[WT][Proxy] Metadata listener error: {str(e)}")
            ...
            if response.status == 200:
                if True:
                    async for chunk_bytes in response.content.iter_chunked(1024):
                        # print(f'[WT] ### chunk_bytes: {chunk_bytes}')
                        yield chunk_bytes
                else:
...
async def handle_request():
    try:
        ...
        # print(f'[WT] %%% prefill_request: {prefill_request}')
        ...
        # print(f'[WT] %%% response: {dir(response.response.iter)}')
...
if __name__ == "__main__":
    ...
    # [WT][ADD] predictor metadata listener
    t2 = threading.Thread(
