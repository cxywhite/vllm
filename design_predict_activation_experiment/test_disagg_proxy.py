...
class DecodeMonitor:
    ...
    #     """
    #     Two-stage decode instance selection:
    #     Stage 1: select Top-K instances with smallest instantaneous load (Load_Bottle)
    #     Stage 2: among Top-K, select instance with minimal future workload
    #             (running_predict_tokens + waiting_tokens)
    #     """
    ...
    #         # --- Stage 1 metric: instantaneous 3D load ---
    #         load_dict = self.calculate_load(
    #             n_requests=running_requests,
    #             m_running_tokens=running_tokens,
    #         )
    ...
    #         # --- Stage 2 metric: future workload ---
    #         future_tokens = (
    #             int(stat.get("running_predict_tokens", 0))
    #             + int(stat.get("waiting_tokens", 0))
    #         )
    ...
    #         # fallback
    #         addr = available_instances[0]
    #         print(f"[WT][SCHED] fallback select {addr}", flush=True)
    #         return addr, float("inf")

    #     # ---------- Stage 1: Top-K by instantaneous load ----------
    #     candidates.sort(key=lambda x: x["load_now"])
    #     K = min(len(candidates), 2)
    #     topk = candidates[:K]

    #     # ---------- Stage 2: select minimal future workload ----------
    #     best = min(topk, key=lambda x: x["future_tokens"])

    #     print(
    #         "[WT][SCHED] select decode instance:\n"
    #         f"  addr={best['addr']}\n"
    #         f"  load_now={best['load_now']:.4f}\n"
    #         f"  future_tokens={best['future_tokens']}\n"
    #         f"  load_detail={best['load_dict']}",
    #         flush=True,
    #     )
