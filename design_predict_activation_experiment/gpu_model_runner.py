...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def execute_model(
        ...
        if self.scheduler_config.activation_predict:
            # 是否需要提取req_id 先不简化
            # import re
            # def extract_simplified_req_id(full_req_id: str) -> str | None:
            #     """
            #     从完整 req_id 提取简化 req_id，
            #     例如 '...uuid-0' -> 'uuid-0'
            #     """
            #     uuid_pattern = r'([a-f0-9]{32}-\d+)$'
            #     match = re.search(uuid_pattern, full_req_id)
            #     if match:
            ...
            for req in scheduled_new_reqs:
                ...
                simplified_req_id = full_req_id  # 直接使用完整 req_id 作为 key
                # simplified_req_id = extract_simplified_req_id(full_req_id)
                # if simplified_req_id is None:
                #     # 如果无法提取简化 id，就用原始 id 作为 key
