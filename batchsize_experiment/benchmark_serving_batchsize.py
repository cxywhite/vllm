...
def main(args: argparse.Namespace):
    ...
    # 加载之前保存的接送文件作为input_requests
    if args.load_input_requests:
        # 判断是否文件存在
        if not os.path.isfile(args.load_input_requests):
            raise ValueError(
                f"Input requests file {args.load_input_requests} does not exist."
            )
        with open(args.load_input_requests, "r", encoding="utf-8") as f:
            input_requests_json = json.load(f)
        input_requests = []
        for ir in input_requests_json:
            input_requests.append(
                SampleRequest(
                    expected_output_len=ir.get("expected_output_len", 2048),
                    lora_request=ir.get("lora_request", None),
                    multi_modal_data=ir.get("multi_modal_data", None),
                    prompt=ir["prompt"],
                    prompt_len=ir["prompt_len"],
                    repetition_penalty=ir.get("repetition_penalty", 1.05),
                    req_id=ir.get("req_id", None),
                    temperature=ir.get("temperature", 0.7),
                    top_k=ir.get("top_k", 20),
                    top_p=ir.get("top_p", 0.8),
                )
            )
    else:
        if args.dataset_name == "custom":
            ...
            try:
                ...
                # 保存数据到本地json文件每个input_request包括'expected_output_len', 'lora_request', 'multi_modal_data', 'prompt', 'prompt_len', 'repetition_penalty', 'req_id', 'temperature', 'top_k', 'top_p'
                if args.save_input_requests:
                    input_requests_list = [
                        {
                            "expected_output_len": ir.expected_output_len,
                            "lora_request": ir.lora_request,
                            "multi_modal_data": ir.multi_modal_data,
                            "prompt": ir.prompt,
                            "prompt_len": ir.prompt_len,
                        ...
                        for ir in input_requests
                    ]
                    # 时间戳后缀命名
                    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
                    os.makedirs("/root/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/experiment_result/dataset", exist_ok=True)
                    save_path = f"/root/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/experiment_result/dataset/input_requests_{args.dataset_name}_{timestamp}.json"
                    with open(save_path, "w", encoding="utf-8") as f:
                        json.dump(input_requests_list, f, ensure_ascii=False, indent=4)
                    print(f"Saved input requests to {save_path}")
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a json file containing previously saved "
        "input requests to load and use for the benchmark.",
    )
    parser.add_argument(
        "--save-input-requests",
        action="store_true",
        help="Whether to save the generated input requests "
        "to a local json file named 'input_requests.json'.",
    )
