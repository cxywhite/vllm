...
from backend_request_func_mean_and_sigma import (
    ASYNC_REQUEST_FUNCS,
...
try:
    ...
    from backend_request_func_mean_and_sigma import get_tokenizer
...
from benchmark_dataset_mean_and_sigma import (
    AIMODataset,
    ASRDataset,
    BurstGPTDataset,
    ConversationDataset,
    ...
    if args.dataset_name == "custom" or args.dataset_name == "mysharegpt" or args.dataset_name == "lmsyschat" or args.dataset_name == "qwen" or args.dataset_name == "qwen_new":
        if args.dataset_name == "lmsyschat":
            ...
            for idx, row in df.iterrows():
                ...
                mean_len=int(row['mean_len'])
                if args.test_model == "llama":
                    if args.test_max_outputlen:
                        # print("LLaMA detected, setting expected_output_len to 8192 - prompt_tokens")
                        expected_output_len=8192-int(prompt_tokens)
                        print(f"LLaMA detected, setting expected_output_len to {expected_output_len} (8192 - prompt_tokens)")
                    else:
                        expected_output_len=row['output_tokens']
                        print(f"LLaMA detected, setting expected_output_len to {expected_output_len} (from dataset)")
                elif args.test_model == "qwen":
                    if args.test_max_outputlen:
                        # print("Qwen detected, setting expected_output_len to 32768 - prompt_tokens")
                        expected_output_len=32768-int(prompt_tokens)
                        print(f"Qwen detected, setting expected_output_len to {expected_output_len} (32768 - prompt_tokens)")
                    else:
                        expected_output_len=row['output_tokens']
                        print(f"Qwen detected, setting expected_output_len to {expected_output_len} (from dataset)")
            ...
            for i, req in enumerate(input_requests[:12]):
                print(f"Request {i}: req_id={req.req_id}, prompt_len={req.prompt_len}, "
                    f"expected_output_len={req.expected_output_len}, mean_len={req.mean_len}, "
            ...
            # shuffle the dataframe to ensure randomness
            # df = df.sample(frac=1, random_state=args.seed).reset_index(drop=True)
            # randomly select num_prompts rows
            df = df.head(args.num_prompts)
            input_requests: List[SampleRequest] = []
            for idx, row in df.iterrows():
                req_id=row['req_id']
            ...
            # 打印前几条请求进行验证
            for i, req in enumerate(input_requests[:6]):
                print(f"Request {i}: req_id={req.req_id}, prompt_len={req.prompt_len}, "
                    f"expected_output_len={req.expected_output_len}, mean_len={req.mean_len}, "
                    f"temperature={req.temperature}, top_p={req.top_p}, "
                    f"top_k={req.top_k}, repetition_penalty={req.repetition_penalty}")
...
def create_argument_parser():
    ...
    # 加test-max-outputlen替换test-optimal
    parser.add_argument("--test-max-outputlen",action='store_true',help="test with max output length or not")
    # parser.add_argument("--test-optimal",action='store_true',help="p2d vllm or not")
