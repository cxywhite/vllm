...
from backend_request_func_dataset import (
    ASYNC_REQUEST_FUNCS,
...
from benchmark_dataset_dataset import (
    AIMODataset,
    ASRDataset,
    BurstGPTDataset,
    ConversationDataset,
...
vllm_root=os.path.dirname(current_dir)
wt_test_dir=os.path.join(vllm_root, "wt_test")
sys.path.insert(0,wt_test_dir)
from customfunction import save_outputs
@dataclass
...
async def benchmark(
    ...
    sampling_params = {
                "max_tokens": 10,
                "temperature": input_requests[0].temperature,
                "top_p": input_requests[0].top_p,
                "top_k": input_requests[0].top_k,
                "repetition_penalty": input_requests[0].repetition_penalty,
            }
    assert (
    ...
    test_input = RequestFuncInput(
        ...
        req_id=input_requests[0].req_id
    )
    ...
    if args.save_output:
        ...
        print(f"Saved detailed outputs to: {result_path}")
    if pbar is not None:
        pbar.close()
...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom" or args.dataset_name == "mysharegpt" or args.dataset_name == "lmsyschat":
        ...
        for idx, row in df.iterrows():
            ...
            prompt_tokens=row['input_length']
            ...
            expected_output_len=row['custom_max_tokens']  # assuming max total tokens is 32768
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "burstgpt", "sonnet", "random", "hf", "custom","mysharegpt","lmsyschat"],
    ...
    parser.add_argument("--save-output",action='store_true',help="是否保存输出结果")
