...
from customfunction import load_and_prepare_dataset,ensure_id_column_from_df,extract_first_human_prompt,save_outputs_grouped_csv_by_base_reqid,save_test_outputs,save_full_outputs
import pdb
...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom" or args.dataset_name == "mysharegpt":
        ...
        if (args.ablation_p2d or args.reproduce_baseline) and args.baseline_csv_path:
            test_df = pd.read_csv(args.baseline_csv_path)
            req_id_to_output_len = {
                str(row['req_id']): int(row['output_tokens'])
            ...
            for request in input_requests:
                ...
                if rid in req_id_to_output_len:
                    request.expected_output_len = req_id_to_output_len[rid]
                    total_num += 1
                else:
                    print(f"Warning: req_id {rid} not found in baseline CSV")

            assert total_num == len(input_requests)
            print(f"已根据 baseline CSV 更新 {total_num} 条请求的 expected_output_len")
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "burstgpt", "sonnet", "random", "hf", "custom","mysharegpt"],
    ...
    parser.add_argument("--min-prompt-tokens",type=int,default=0,help="最小输入长度")
