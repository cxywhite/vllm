...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom":
        ...
        if args.dataset_name == "custom":
            ...
            startidx = getattr(args, 'startidx', 0)
            count = getattr(args, 'count', 0)
            assert isinstance(startidx, int) and startidx >= 0 and ((isinstance(count, int) and count > 0)), \
                "请正确设置 startidx (>=0) 和 count (>0)"
            ds= ds.select(range(startidx, startidx + count))
            try:
                df = ds.to_pandas()
            except Exception:
                rows = []
        ...
        df['custom_max_tokens'] = df['prompt_tokens'].apply(
        ...
        for idx, row in df.iterrows():
            ...
            if not base_id:
                base_id = f"row{idx}"
            m=getattr(args, 'm', 0)
            # assert isinstance(m, int) and m == 50, "请正确设置 m=50"
            # 每个样本重复 m 次，生成不同
            for i in range(m): 
                input_requests.append(
                    SampleRequest(
                        req_id=base_id+f'r{i}',
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["sharegpt", "burstgpt", "sonnet", "random", "hf", "custom","mysharegpt","lmsyschat","qwen"],
    ...
    parser.add_argument("--startidx",type=int,default=0,help="custom dataset start index")
    parser.add_argument("--count",type=int,default=100,help="custom dataset count")
    return parser
