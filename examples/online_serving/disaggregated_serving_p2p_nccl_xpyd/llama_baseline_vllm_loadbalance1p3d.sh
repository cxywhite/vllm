...
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/Loadbalance/dataset/llama-lmsys-chat-loadbalance_1p3d_2.csv}
...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-./wt_experiment/experiment_paper/tmp/baseline_227}
...
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"6,7,8,9,10,11,12"}
...
elif [ "$TEST_OPTIMAL" = "true" ]; then
    PROXY_SCRIPT=${PROXY_SCRIPT:-test_disagg_proxy_p2d_loadbalance.py}
...
setup_directories() {
    ...
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_np${num_prompts}_rr${req_rate}_mt${max_tokens}_${timestamp}_1p3d_rr"
...
start_servers() {
    ...
    for i in "${!DECODE_GPU_ARRAY[@]}"; do
        ...
        if [ "$TEST_OPTIMAL" = "true" ]; then CMD="$CMD --test-optimal"; fi
