...
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/3DLoad/3Dtestdataset}
...
# SWEEP CONFIGURATION
# ==========================================
NUM_PROMPTS_LIST=${NUM_PROMPTS_LIST:-"5000"}             
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"100"}
...
setup_directories() {
    ...
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_np${num_prompts}_rr${req_rate}_${timestamp}_1p1d"
...
main() {
    ...
    IFS=',' read -ra REQUEST_RATE_ARRAY <<< "$BENCH_REQUEST_RATE_LIST"
        ...
        for num_prompts in "${NUM_PROMPTS_ARRAY[@]}"; do
            ...
            setup_directories "$num_prompts" "$req_rate" "$timestamp"
            
            echo "========================================"
            echo "Run: np=${num_prompts} rr=${req_rate}"
            echo "Dir: ${BENCHMARK_DIR}"
            echo "========================================"
            
            dump_config_json "$num_prompts" "$req_rate" "$timestamp"
            ...
            log_file="${LOG_DIR}/bench_np${num_prompts}_rr${req_rate}.log"
