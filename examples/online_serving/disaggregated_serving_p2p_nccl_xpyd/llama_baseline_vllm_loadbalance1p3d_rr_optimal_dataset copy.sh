...
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/experiment_paper/Mean_and_sigma/dataset/llama_mean_and_sigma_merged_50_with_stats.csv}
...
find_latest_rr_benchmark_dir() {
    local num_prompts=$1
    local req_rate=$2
    local max_tokens=$3
    local repeat_idx=$4
    local pattern="${RR_RESULT_ROOT}/benchmark_np${num_prompts}_rr${req_rate}_mt${max_tokens}_*_rep${repeat_idx}_1p3d_rr"
...
build_optimal_dataset() {
    ...
    python3 - "$source_dataset" "$rr_csv" "$output_dataset" <<'PY'
import sys
import pandas as pd

source_path, rr_path, output_path = sys.argv[1:4]
base_df = pd.read_csv(source_path)
rr_df = pd.read_csv(rr_path)

required_base = {"req_id", "output_tokens"}
required_rr = {"req_id", "output_tokens"}
...
if not required_rr.issubset(rr_df.columns):
    missing = sorted(required_rr - set(rr_df.columns))
    raise ValueError(f"RR result CSV missing required columns: {missing}")

rr_map = rr_df[["req_id", "output_tokens"]].dropna(subset=["req_id"]).drop_duplicates(subset=["req_id"], keep="last")
rr_map["req_id"] = pd.to_numeric(rr_map["req_id"], errors="coerce")
base_df["req_id"] = pd.to_numeric(base_df["req_id"], errors="coerce")

merged = base_df.merge(
    rr_map.rename(columns={"output_tokens": "rr_output_tokens"}),
    on="req_id",
    how="left",
)
merged["output_tokens"] = merged["rr_output_tokens"].combine_first(merged["output_tokens"])
merged = merged.drop(columns=["rr_output_tokens"])
...
run_single_benchmark() {
    ...
    if is_true "$TEST_OPTIMAL" && [[ "$dataset_path" == "__AUTO_OPTIMAL__" ]]; then
        local rr_benchmark_dir
        local rr_result_dir
        local rr_results_csv

        if ! rr_benchmark_dir=$(find_latest_rr_benchmark_dir "$num_prompts" "$req_rate" "$max_tokens" "$repeat_idx"); then
            echo "Cannot find matching rr directory in ${RR_RESULT_ROOT} for np=${num_prompts}, rr=${req_rate}, mt=${max_tokens}, rep=${repeat_idx}."
            return 1
        fi

        rr_result_dir="${rr_benchmark_dir}/dataset_result"
        if ! rr_results_csv=$(find_latest_test_results_csv "$rr_result_dir"); then
            echo "No rr test_results_*.csv found in ${rr_result_dir}."
            return 1
        fi

        CURRENT_RR_RESULTS_CSV="$rr_results_csv"
        effective_dataset_path=$(build_optimal_dataset "$BENCH_DATASET_PATH" "$rr_results_csv" "$BENCHMARK_DIR" "$timestamp")
        echo "Generated optimal dataset: ${effective_dataset_path}"
    fi
    ...
    # Optimal runs always benchmark with --ignore-eos.
    if is_true "$TEST_OPTIMAL"; then
...
main() {
    ...
    mkdir -p "$RR_RESULT_ROOT" "$OPTIMAL_RESULT_ROOT"
        ...
        for req_rate in "${REQUEST_RATE_ARRAY[@]}"; do
            for max_tokens in "${MAX_TOKENS_ARRAY[@]}"; do
                ...
                for repeat_idx in $(seq 1 "$RUN_REPEAT_PER_CONFIG"); do
                    ...
                    if ! run_single_benchmark "$num_prompts" "$req_rate" "$max_tokens" "$repeat_idx" "false" "$BENCH_DATASET_PATH"; then
                        echo "RR benchmark failed."
                        cleanup
                        exit 1
                    fi

                    if ! run_single_benchmark "$num_prompts" "$req_rate" "$max_tokens" "$repeat_idx" "true" "__AUTO_OPTIMAL__"; then
