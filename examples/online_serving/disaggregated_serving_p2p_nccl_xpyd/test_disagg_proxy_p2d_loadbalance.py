...
class DecodeMonitor:
    ...
    def select_best_instance(self, instances, predictor_meta: Optional[dict]=None):
        ...
        for addr, zmq_addr in instances:
            ...
            if load and "Load_Bottle" in load and "Future_Tokens" in load:
                valid_instances.append((
                    load["Future_Tokens"]+predictor_meta['predict_output_len']+predictor_meta['prompt_len'],  # 用于第一轮排序load["Future_Tokens"]+predictor_meta['predict_output_len'],
        ...
        # 按Future_tokens排序取top1
        valid_instances.sort(key=lambda x: x[0])  # 按Future_tokens升序排序
        top_instance = valid_instances[0]  # Future_tokens最小的实例
...
async def handle_request():
    try:
        ...
        predictor_meta=Custom_Metadata(req_id=None,token_range=None,num_tokens=None,temperature=None,top_p=None,top_k=None,repetition_penalty=None,prompt_len=original_request_data.get('prompt_len'),predict_output_len=original_request_data.get('max_tokens')).to_dict()
        # print(f"Received request, predictor_meta: {predictor_meta}")
