...
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/Loadbalance/dataset/qwen-mysharegpt-loadbalance_1p3d.csv}
BENCH_DATASET_PATH_LIST=${BENCH_DATASET_PATH_LIST:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/experiment_paper/Loadbalance/dataset/qwen-lmsys-chat-loadbalance_1p3d_good.csv}
...
BASE_RESULT_DIR_PREFIX=${BASE_RESULT_DIR_PREFIX:-./wt_experiment/experiment_paper/tmp/baseline_302/1p3d_qwen_lmsyschat}
BASE_RESULT_DIR=${BASE_RESULT_DIR:-$BASE_RESULT_DIR_PREFIX}
...
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"6,8,10"}
...
update_base_result_dir_by_dataset_path() {
    local dataset_path="$1"
    local dataset_base dataset_idx dataset_stem

    dataset_base="$(basename "$dataset_path")"
    dataset_idx="$(echo "$dataset_base" | sed -n 's/.*_\([0-9]\+\)\.csv$/\1/p')"

    if [[ -n "$dataset_idx" ]]; then
        BASE_RESULT_DIR="${BASE_RESULT_DIR_PREFIX}_${dataset_idx}"
    else
        dataset_stem="${dataset_base%.csv}"
        dataset_stem="$(echo "$dataset_stem" | tr '/ ' '__')"
        BASE_RESULT_DIR="${BASE_RESULT_DIR_PREFIX}_${dataset_stem}"
    fi
}
...
setup_directories() {
    ...
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_np${num_prompts}_rr${req_rate}_mt${max_tokens}_${timestamp}_rep${repeat_idx}_${dir_suffix}"
...
main() {
    ...
    IFS=',' read -ra DATASET_PATH_ARRAY <<< "$BENCH_DATASET_PATH_LIST"
    ...
    for dataset_path in "${DATASET_PATH_ARRAY[@]}"; do
        dataset_path="$(trim_spaces "$dataset_path")"
        [[ -z "$dataset_path" ]] && continue
        [[ -f "$dataset_path" ]] || { echo "Dataset file not found: $dataset_path"; exit 1; }

        export BENCH_DATASET_PATH="$dataset_path"
        update_base_result_dir_by_dataset_path "$BENCH_DATASET_PATH"

        echo "========================================"
        echo "Dataset: ${BENCH_DATASET_PATH}"
                ...
                for max_tokens in "${MAX_TOKENS_ARRAY[@]}"; do
                    for optimal_mode in false true; do
                        ...
                        for repeat_idx in $(seq 1 "$RUN_REPEAT_PER_CONFIG"); do
                            ...
                            # 如果request rate为10 并且optimal_mode为false，跳过
                            if [ "$req_rate" -eq 10 ] && [ "$TEST_OPTIMAL" = "false" ]; then
                                echo "Skipping suboptimal config with request rate 10"
                                continue
                            fi
                            # 如果request rate为10 并且optimal_mode为true并且repeat_idx为1，跳过
                            if [ "$req_rate" -eq 10 ] && [ "$TEST_OPTIMAL" = "optimal" ] && [ "$repeat_idx" -eq 1 ]; then
                                echo "Skipping suboptimal config with request rate 10"
                                continue
                            fi
