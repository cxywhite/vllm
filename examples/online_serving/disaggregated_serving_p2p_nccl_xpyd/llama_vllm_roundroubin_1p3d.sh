...
BENCH_DATASET_NAME=${BENCH_DATASET_NAME:-auto}
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/.cache/huggingface/hub/datasets/wt_predictor/test_baselinedataset/llama}
BENCH_DATASET_RECURSIVE=${BENCH_DATASET_RECURSIVE:-true}
BENCH_NUM_PROMPTS=${BENCH_NUM_PROMPTS:-2000}
BENCH_BURSTINESS=${BENCH_BURSTINESS:-1}
BENCH_REQUEST_RATES=${BENCH_REQUEST_RATES:-4,6,8,10}
BENCH_GOODPUT_TTFT_MS=${BENCH_GOODPUT_TTFT_MS:-1000}
BENCH_GOODPUT_TPOT_MS_LIST=${BENCH_GOODPUT_TPOT_MS_LIST:-30,50,100}
...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-$(dirname "${BASH_SOURCE[0]}")/wt_experiment/baseline_runs}
LOG_SUFFIX=${LOG_SUFFIX:-}

BENCH_DATASET_TAG=$(basename "$BENCH_DATASET_PATH" | sed -E 's/[^[:alnum:]_.-]+/-/g; s/^-+//; s/-+$//; s/-+/-/g')
BENCH_RATE_TAG=$(echo "$BENCH_REQUEST_RATES" | sed -E 's/[ ,\/]+/-/g; s/^-+//; s/-+$//; s/-+/-/g; s/\./p/g')
BENCH_TPOT_TAG=$(echo "$BENCH_GOODPUT_TPOT_MS_LIST" | sed -E 's/[ ,\/]+/-/g; s/^-+//; s/-+$//; s/-+/-/g; s/\./p/g')

[ -z "$BENCH_DATASET_TAG" ] && BENCH_DATASET_TAG="na"
[ -z "$BENCH_RATE_TAG" ] && BENCH_RATE_TAG="na"
[ -z "$BENCH_TPOT_TAG" ] && BENCH_TPOT_TAG="na"

RUNTIME_LOG_SUFFIX="ds${BENCH_DATASET_TAG}_rr${BENCH_RATE_TAG}_tpot${BENCH_TPOT_TAG}"
...
echo "  Dataset Name Mode: $BENCH_DATASET_NAME"
echo "  Dataset Path: $BENCH_DATASET_PATH"
echo "  Dataset Recursive: $BENCH_DATASET_RECURSIVE"
echo "  Request Rates: $BENCH_REQUEST_RATES"
echo "  TPOT List (ms): $BENCH_GOODPUT_TPOT_MS_LIST"
...
resolve_dataset_paths() {
    ...
    if [[ -d "$BENCH_DATASET_PATH" ]]; then
        local find_args=("$BENCH_DATASET_PATH" -type f -name '*.csv')
        if ! is_truthy "$BENCH_DATASET_RECURSIVE"; then
            find_args=("$BENCH_DATASET_PATH" -maxdepth 1 -type f -name '*.csv')
        fi
        while IFS= read -r csv_file; do
            DATASET_PATH_ARRAY+=("$csv_file")
        done < <(find "${find_args[@]}" | sort)

        if [[ ${#DATASET_PATH_ARRAY[@]} -eq 0 ]]; then
            echo "No CSV files found under dataset directory: $BENCH_DATASET_PATH"
            exit 1
        fi
    elif [[ -f "$BENCH_DATASET_PATH" ]]; then
        DATASET_PATH_ARRAY=("$BENCH_DATASET_PATH")
    else
        echo "Dataset path not found: $BENCH_DATASET_PATH"
        exit 1
    fi
}
...
set_current_dataset() {
    ...
    local dataset_label_source

    CURRENT_BENCH_DATASET_PATH="$dataset_path"
    if [[ -d "$BENCH_DATASET_PATH" ]]; then
        dataset_label_source="${dataset_path#"$BENCH_DATASET_PATH"/}"
    else
        dataset_label_source="$(basename "$dataset_path")"
    fi
    CURRENT_BENCH_DATASET_LABEL="$(sanitize_name "${dataset_label_source%.csv}")"
}
    ...
    cat > "$RUN_CONFIG_JSON" <<EOF
{
    ...
    "bench_dataset_recursive": "$BENCH_DATASET_RECURSIVE",
    "bench_request_rates": "$BENCH_REQUEST_RATES",
    "bench_goodput_tpot_ms_list": "$BENCH_GOODPUT_TPOT_MS_LIST",
...
start_proxy_server() {
    local tag="$1"
    local current_tpot_ms="${2:-$BENCH_GOODPUT_TPOT_MS}"
    PROXY_LOG="$LOG_DIR/proxy_${RUN_TIMESTAMP}_${tag}.log"
    local proxy_model_config_path="${MODEL}/config.json"
    local proxy_ttft="ttft:${BENCH_GOODPUT_TTFT_MS}"
    local proxy_tpot="tpot:${current_tpot_ms}"
    local proxy_dtype="bfloat16"

    echo "Starting proxy server on port $PROXY_PORT (tag=$tag)..."
    echo "Proxy self-check env: MODEL_CONFIG_PATH=$proxy_model_config_path TTFT=$proxy_ttft TPOT=$proxy_tpot VLLM_DTYPE=$proxy_dtype"
    PROXY_PORT=$PROXY_PORT BENCH_PORT=$BENCH_PORT QUART_DEBUG=$QUART_DEBUG MODEL_CONFIG_PATH=$proxy_model_config_path TTFT=$proxy_ttft TPOT=$proxy_tpot VLLM_DTYPE=$proxy_dtype python3 disagg_proxy_p2p_nccl_xpyd.py \
            > "$PROXY_LOG" 2>&1 &
    PROXY_PID=$!
...
restart_proxy_server() {
    ...
    local current_tpot_ms="${2:-$BENCH_GOODPUT_TPOT_MS}"
    stop_proxy_server
    start_proxy_server "$tag" "$current_tpot_ms"
}
...
restart_all_services_and_recover() {
    ...
    if ! start_proxy_server "recover_${tag}" "$current_tpot_ms"; then
        return 1
    fi
    return 0
}
...
main() {
    ...
    local tpot_values_raw="${BENCH_GOODPUT_TPOT_MS_LIST//,/ }"
    local request_rates=()
    local tpot_values=()
    read -r -a request_rates <<< "$request_rates_raw"
    read -r -a tpot_values <<< "$tpot_values_raw"

    if [ "${#request_rates[@]}" -eq 0 ] || [ "${#tpot_values[@]}" -eq 0 ]; then
        echo "BENCH_REQUEST_RATES / BENCH_GOODPUT_TPOT_MS_LIST is empty."
        cleanup 1
    fi
    ...
    echo "  dataset_count: ${#DATASET_PATH_ARRAY[@]}"
    echo "  dataset_name_mode: $BENCH_DATASET_NAME"
    echo "  request_rates: ${request_rates[*]}"
    echo "  tpot_ms_list: ${tpot_values[*]}"
    ...
    for dataset_path in "${DATASET_PATH_ARRAY[@]}"; do
        ...
        for request_rate in "${request_rates[@]}"; do
            for tpot_ms in "${tpot_values[@]}"; do
                ...
                echo ""
                echo "[Benchmark $total_runs] dataset=${CURRENT_BENCH_DATASET_PATH}, request_rate=$request_rate, tpot_ms=$tpot_ms"
                echo "Log: $combo_log"
                ...
                python3 "$BENCH_SCRIPT" \
                    ...
                    --goodput "ttft:$BENCH_GOODPUT_TTFT_MS" "tpot:$tpot_ms" \
                    --ignore-eos \
                    > "$combo_log" 2>&1 &
                ...
                if (( bench_exit_code == 124 )); then
                    ...
                    echo "dataset=${CURRENT_BENCH_DATASET_PATH}, request_rate=$request_rate, tpot_ms=$tpot_ms, status=watchdog_timeout, bottleneck=$CURRENT_BOTTLENECK, log=$combo_log, proxy_log=$combo_proxy_log" | tee -a "$summary_log"
                    if ! restart_all_services_and_recover "$combo_tag" "$tpot_ms"; then
                ...
                analyze_proxy_bottleneck "$combo_proxy_log" "$combo_tag" | tee -a "$summary_log" "$combo_log"
                echo "dataset=${CURRENT_BENCH_DATASET_PATH}, request_rate=$request_rate, ttft_ms=$BENCH_GOODPUT_TTFT_MS, tpot_ms=$tpot_ms, exit_code=$bench_exit_code, bottleneck=$CURRENT_BOTTLENECK, log=$combo_log, proxy_log=$combo_proxy_log" | tee -a "$summary_log"
