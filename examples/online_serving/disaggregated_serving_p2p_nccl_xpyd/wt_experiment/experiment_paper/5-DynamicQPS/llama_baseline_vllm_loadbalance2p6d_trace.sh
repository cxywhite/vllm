...
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/myshare/DynamicQPS/qwen/}
...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/experiment_paper/tmp/baseline_317}
            ...
            for max_tokens in "${MAX_TOKENS_ARRAY[@]}"; do
                for optimal_mode in false true; do
                    ...
                    if is_true "$TEST_OPTIMAL"; then
                        ...
                        # if [ "$dataset_path" = "/root/myshare/DynamicQPS/burstgpt/A_gt_small_1_dataset.csv" ]; then
