...
# Use paired rep-to-rep matching and max(opt/rr) for each request rate.
QPS_LIST = [6, 8, 10, 12]
...
def paired_max_ratio_by_qps(metric, qps, rr_dir, opt_dir):
            ...
            for ds_label, ds_dir_key in DATASETS.items():
                ...
                for qps in QPS_LIST:
                    ...
                    if rr_dir is None or opt_dir is None:
                        ...
                        row_data[col_name] = paired_max_ratio_by_qps(metric, qps, rr_dir, opt_dir)
...
def draw_tight_academic_table(df):
    ...
    save_path = os.path.join(save_dir, f"table_three_color_repmax_{datetime.now().strftime('%H%M%S')}.png")
