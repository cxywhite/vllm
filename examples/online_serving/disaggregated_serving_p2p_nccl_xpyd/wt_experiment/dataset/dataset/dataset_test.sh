...
# 路径配置
DATASET_DIR="/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/dataset/dataset/qwen_dataset/lmsys-chat/split"
BENCHMARK_SCRIPT="/root/predict-schedule/vllm/benchmarks/benchmark_serving_dataset.py"
OUTPUT_ROOT="/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/dataset/dataset/qwen_dataset/lmsys-chat/processed_output"
...
if ! wait_for_proxy_ready "$PROXY_PORT" "Proxy-Server"; then
    echo "Proxy 无法正常工作，请检查 proxy.py 日志。"
    cleanup
fi
...
files=$(ls "$DATASET_DIR"/lmsys-chat_preprocessed_prompts_part_*.csv 2>/dev/null | sort -V)
...
for file_path in $files; do
    ...
    python3 "$BENCHMARK_SCRIPT" \
        --backend openai-chat \
        --model "/root/.cache/huggingface/hub/Qwen2.5-7B-Instruct" \
        --endpoint /v1/chat/completions \
        --host 127.0.0.1 \
        --port "$PROXY_PORT" \
        --dataset-name lmsyschat \
