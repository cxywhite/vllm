...
import pdb
import os
dataset_path = f'/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/dataset/dataset/qwen_dataset/lmsys-chat/split'
...
for i in range(0, len(df), 16000):
    ...
    # 打印前5行数据进行检查
    print(df_part.head())
    df_part.to_csv(f'/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/wt_experiment/dataset/dataset/qwen_dataset/lmsys-chat/split/qwen_chat_preprocessed_prompts_part_{i//16000}.csv', index=False)
