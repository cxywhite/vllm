...
BENCH_DATASET_NAME=${BENCH_DATASET_NAME:-mysharegpt} #custom mysharegpt
BENCH_DATASET_PATH=${BENCH_DATASET_PATH:-/root/.cache/huggingface/hub/datasets--shibing624--sharegpt_gpt4/snapshots/3fb53354e02a931777556fb1da37e931d73af48a} #/root/predict-schedule/baseline_experiment/pastfuture/dataset/lmsys-50k-filtered-with-newid-newsample /root/.cache/huggingface/hub/datasets--shibing624--sharegpt_gpt4/snapshots/3fb53354e02a931777556fb1da37e931d73af48a/sharegpt_gpt4.jsonl
...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-./experiment_result/experiment_paper/tmp/baseline}
# Proxy
...
setup_directories() {
    ...
    # Create base benchmark directory with num_prompts and timestamp
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_np${num_prompts}_${timestamp}"
...
start_servers() {
    ...
    for i in "${!PREFILL_GPU_ARRAY[@]}"; do
        ...
        setsid env CUDA_VISIBLE_DEVICES="$gpu_id" VLLM_USE_V1=1 bash -c "exec $CMD" \
            > "${LOG_DIR}/prefill${i}_${timestamp}.log" 2>&1 &
        pid=$!
    ...
    for i in "${!DECODE_GPU_ARRAY[@]}"; do
        ...
        setsid env DECODE_INSTANCE_ID="decode-${i}" VLLM_USE_V1=1 CUDA_VISIBLE_DEVICES="$gpu_id" bash -c "exec $CMD" \
            > "${LOG_DIR}/decode${i}_${timestamp}.log" 2>&1 &
        pid=$!
        pgid=$(ps -o pgid= -p "$pid" | tr -d ' ')
...
main() {
    ...
    for num_prompts in "${NUM_PROMPTS_ARRAY[@]}"; do
        ...
        # Setup directories for this benchmark run
        setup_directories "$num_prompts" "$timestamp"
        
        echo "========================================"
        echo "Run timestamp=${timestamp}  num-prompts=${num_prompts}"
        echo "Benchmark directory: ${BASE_RESULT_DIR}/benchmark_np${num_prompts}_${timestamp}"
        echo "========================================"
        
        # Dump config after directories are created
        dump_config_json
        ...
        CMD="python3 \"$BENCH_SCRIPT\" \
            ...
            --maxtokenscustom 2000 --seed ${VLLM_SEED} \
        ...
        # 启动基准测试
        setsid env BENCHMARK_INSTANCE_ID="benchmark-${num_prompts}" bash -c "exec $CMD" \
                > "$log_file" 2>&1 &
        client_pid=$!
