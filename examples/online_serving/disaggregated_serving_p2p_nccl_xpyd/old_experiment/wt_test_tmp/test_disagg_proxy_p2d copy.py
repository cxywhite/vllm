...
class DecodeMonitor:
    ...
    def select_best_instance(self, instances, predictor_meta: Optional[dict]=None):
        ...
        top2_instances = valid_instances[:2]
        
        # 在top2/3中按Load_Bottle排序取最小
        top2_instances.sort(key=lambda x: x[1])   # 按Load_Bottle升序
        best_future, best_bottle, best_addr, best_zmq_addr = top2_instances[0]
...
async def handle_request():
    try:
        ...
        predictor_meta=Custom_Metadata(req_id=None,token_range=None,num_tokens=None,temperature=None,top_p=None,top_k=None,repetition_penalty=None,predict_output_len=original_request_data['max_tokens']).to_dict()
