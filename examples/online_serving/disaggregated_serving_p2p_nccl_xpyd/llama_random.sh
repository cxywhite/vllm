...
PROXY_PORT=${PROXY_PORT:-28302}
BENCH_PORT=${BENCH_PORT:-23007}

PREFILL_GPUS=${PREFILL_GPUS:-6}
PREFILL_PORTS=${PREFILL_PORTS:-23001}
PREFILL_KV_PORTS=${PREFILL_KV_PORTS:-23011}
PREFILL_GPU_MEMORY_UTILIZATION=${PREFILL_GPU_MEMORY_UTILIZATION:-0.9}
...
DECODE_PORTS=${DECODE_PORTS:-23020}
DECODE_KV_PORTS=${DECODE_KV_PORTS:-23032}
...
# SWEEP CONFIGURATION
# ==========================================
RANDOM_INPUT_LEN_LIST=${RANDOM_INPUT_LEN_LIST:-"1024,2048,4096"}
RANDOM_OUTPUT_LEN_LIST=${RANDOM_OUTPUT_LEN_LIST:-"1024,2048,4096"}
NUM_PROMPTS_LIST=${NUM_PROMPTS_LIST:-"500"}             
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"2"}
...
setup_directories() {
    ...
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_in${input_len}_out${output_len}_np${num_prompts}_rr${req_rate}_${timestamp}_1p1d"
...
dump_config_json() {
    local current_input_len=$1
    local current_output_len=$2
    local current_prompts=$3
    local current_rate=$4
    local ts=$5
...
"benchmark": {
    ...
    "random_input_len": ${current_input_len},
    "random_output_len": ${current_output_len},
...
main() {
    ...
    # Parse lists
    IFS=',' read -ra RANDOM_INPUT_LEN_ARRAY <<< "$RANDOM_INPUT_LEN_LIST"
    IFS=',' read -ra RANDOM_OUTPUT_LEN_ARRAY <<< "$RANDOM_OUTPUT_LEN_LIST"
    IFS=',' read -ra NUM_PROMPTS_ARRAY <<< "$NUM_PROMPTS_LIST"
    IFS=',' read -ra REQUEST_RATE_ARRAY <<< "$BENCH_REQUEST_RATE_LIST"
    
    # 4-Layer Nested Loop
    for input_len in "${RANDOM_INPUT_LEN_ARRAY[@]}"; do
      for output_len in "${RANDOM_OUTPUT_LEN_ARRAY[@]}"; do
        for num_prompts in "${NUM_PROMPTS_ARRAY[@]}"; do
          for req_rate in "${REQUEST_RATE_ARRAY[@]}"; do
            # 如果input_len和output_len都为1024，则跳过这个组合（因为之前已经在baseline里跑过了）
            if [[ "$input_len" -eq 1024 && "$output_len" -eq 1024 ]]; then
                echo "Skipping input_len=${input_len} output_len=${output_len} since it's covered in baseline."
                continue
            fi
            timestamp=$(date +%Y%m%d_%H%M%S)
            
            setup_directories "$input_len" "$output_len" "$num_prompts" "$req_rate" "$timestamp"
            
            echo "========================================"
            echo "Run: in=${input_len} out=${output_len} np=${num_prompts} rr=${req_rate}"
            echo "Dir: ${BENCHMARK_DIR}"
            echo "========================================"
            
            dump_config_json "$input_len" "$output_len" "$num_prompts" "$req_rate" "$timestamp"
            ...
            log_file="${LOG_DIR}/bench_in${input_len}_out${output_len}_np${num_prompts}_rr${req_rate}.log"
            ...
            if [ "$REPRODUCE_BASELINE" = "true" ]; then
                CMD="$CMD --reproduce-baseline --baseline-csv-path ${REPRODUCE_BASELINE_CSV_PATH} --ignore-eos"
            fi
            ...
            setsid env BENCHMARK_INSTANCE_ID="benchmark-${num_prompts}-${req_rate}" bash -c "exec $CMD" > "$log_file" 2>&1 &
            client_pid=$!
            wait "$client_pid"
            ...
            for pid in "${PIDS[@]}"; do
                if ! ps -p "$pid" > /dev/null 2>&1; then
                    echo "ERROR: Server died during benchmark."; stop_servers; cleanup; exit 1
                fi
            done
