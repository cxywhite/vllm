...
TEST_ABLATION_DECODE="true"  # 是否测试解码器调度的消融实验
TEST_BASELINE="true"  # 是否测试与基线的对比实验
IGNORE="true"  # 是否忽略EOS标记
BASELINE_CSV_PATH="/root/predict-schedule/vllm/examples/online_serving/disaggregated_serving_p2p_nccl_xpyd/experiment_result/experiment_paper/tmp/preempt_122/benchmark_np100_rr1_mt8192_20260121_202658_1p1d/dataset_result/test_results_20260121_203947.csv"
TEST_MODEL="llama" 
...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-./experiment_result/experiment_paper/tmp/use_aimd_122}
# Proxy
...
PREFILL_PORTS=${PREFILL_PORTS:-25001}
PREFILL_KV_PORTS=${PREFILL_KV_PORTS:-25010}
...
DECODE_PORTS=${DECODE_PORTS:-25020}
DECODE_KV_PORTS=${DECODE_KV_PORTS:-25030}
...
NUM_PROMPTS_LIST=${NUM_PROMPTS_LIST:-"100"}
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"4"}
...
main() {
    ...
    for num_prompts in "${NUM_PROMPTS_ARRAY[@]}"; do
      for req_rate in "${REQUEST_RATE_ARRAY[@]}"; do
        ...
        if [ "$TEST_BASELINE" = "true" ]; then
            echo "baseline-csv-path"
