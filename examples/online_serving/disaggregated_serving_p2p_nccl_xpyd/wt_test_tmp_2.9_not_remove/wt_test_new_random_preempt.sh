...
BASE_RESULT_DIR=${BASE_RESULT_DIR:-./experiment_result/experiment_paper/tmp/preempt_115}

PROXY_PORT=${PROXY_PORT:-28002}
BENCH_PORT=${BENCH_PORT:-22007}

PREFILL_GPUS=${PREFILL_GPUS:-1}
PREFILL_PORTS=${PREFILL_PORTS:-31001}
PREFILL_KV_PORTS=${PREFILL_KV_PORTS:-31011}
...
DECODE_PORTS=${DECODE_PORTS:-31021}
DECODE_KV_PORTS=${DECODE_KV_PORTS:-31031}
...
PREFILL_VLLM_MAX_NUM_BATCHED_TOKENS=${PREFILL_VLLM_MAX_NUM_BATCHED_TOKENS:-8192}
...
NUM_PROMPTS_LIST=${NUM_PROMPTS_LIST:-"1"}
BENCH_REQUEST_RATE_LIST=${BENCH_REQUEST_RATE_LIST:-"1"}
RANDOM_INPUT_LENGTH=${RANDOM_INPUT_LENGTH:-512}
RANDOM_OUTPUT_LENGTH=${RANDOM_OUTPUT_LENGTH:-512}
...
setup_directories() {
    ...
    BENCHMARK_DIR="${BASE_RESULT_DIR}/benchmark_np${num_prompts}_rr${req_rate}_mt${max_tokens}_${timestamp}_1p1d"
...
main() {
    ...
    IFS=',' read -ra RANDOM_INPUT_LENGTH_ARRAY <<< "$RANDOM_INPUT_LENGTH"
    IFS=',' read -ra RANDOM_OUTPUT_LENGTH_ARRAY <<< "$RANDOM_OUTPUT_LENGTH"
    ...
    for num_prompts in "${NUM_PROMPTS_ARRAY[@]}"; do
        for req_rate in "${REQUEST_RATE_ARRAY[@]}"; do
            for random_input_length in "${RANDOM_INPUT_LENGTH_ARRAY[@]}"; do
                for random_output_length in "${RANDOM_OUTPUT_LENGTH_ARRAY[@]}"; do
        
                    timestamp=$(date +%Y%m%d_%H%M%S)
                    
                    setup_directories "$num_prompts" "$req_rate" "$random_output_length" "$timestamp"
                    
                    echo "========================================"
                    echo "Run: Prompts=${num_prompts}, Rate=${req_rate}, MaxTokens=${random_output_length}"
                    echo "Dir: ${BENCHMARK_DIR}"
                    echo "========================================"
                    
                    dump_config_json "$num_prompts" "$req_rate" "$random_output_length" "$timestamp"
                    ...
                    log_file="${LOG_DIR}/bench_np${num_prompts}_rr${req_rate}_mt${random_output_length}.log"
                    ...
                    CMD="python3 \"$BENCH_SCRIPT\" \
                        ...
                        --random-input-len ${random_input_length} \
                        --random-output-len ${random_output_length}"
