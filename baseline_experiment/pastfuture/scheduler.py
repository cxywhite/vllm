...
class SchedulerConfig:
    ...
    pastfuture_scheduler: bool = False
    # [WT] end
    # [WT]predict activation 2025-12-30 16:58:37
    activation_predict: bool = False
    # [WT] end
    ...
    def __post_init__(self) -> None:
        ...
        if self.pastfuture_scheduler:
            logger.info(f"[WT]&&& pastfuture_scheduler")
            self.scheduler_cls = "vllm.v1.core.sched.pastfuture_scheduler.FuturePastScheduler"
        else:
            logger.info(f"[WT]&&& no custom_scheduler")
        # [WT] end
        # [WT]predict activation 2025-12-30 16:58:18
        if self.activation_predict:
            logger.info(f"[WT]&&& activation_predict")
            # self.scheduler_cls = "vllm.v1.core.sched.pastfuture_scheduler.FuturePastScheduler"
        else:
            logger.info(f"[WT]&&& no activation_predict")
        # [WT] end
