...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "custom":
        ...
        # )
        if args.dataset_name == "custom":
            # ---------------- 使用 HuggingFace datasets.load_from_disk() 的自定义分支（替换原 JSONL/JSON 分支） ----------------
            ...
            # 转为 pandas DataFrame（若 to_pandas 可用），否则手动构造
            try:
                df = ds.to_pandas()
                # print(df.columns)
            except Exception:
                ...
                rows = []
                for record in ds:
                    # record 可能为 dict-like 或 ArrowRecord
                    try:
            ...
            # 确保 prompt 为字符串并过滤空 prompt（保留原逻辑）
